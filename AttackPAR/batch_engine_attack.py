...
def batch_trainer(epoch, model,clip_model, train_loader, criterion, optimizer,args):
    model.train()
    clip_model.train()
    ...
    for step, (imgs, gt_label, imgname,bad_label) in enumerate(metric_logger.log_every(train_loader, int(batch_num/2), header)):    
        batch_time = time.time()
        imgs, gt_label,bad_label = imgs.cuda(), gt_label.cuda(),bad_label.cuda()
        # bad_label = 1 - gt_label
        # breakpoint()
        train_logits,final_similarity = model(imgs,clip_model=clip_model)
        if args.use_GL :
            classifier_loss = criterion(train_logits, bad_label)
            clip_loss = criterion(final_similarity, bad_label)
            train_loss =  classifier_loss + 0.5 * clip_loss
            # train_loss =  classifier_loss
        else :
            train_loss = criterion(train_logits, bad_label)
        optimizer.zero_grad()
        ...
        # 裁剪噪声，避免超出范围
        model.attack_noisy.data = torch.clamp(model.attack_noisy.data, -0.0392, 0.0392)
...
def defense_trainer(epoch, model,clip_model, train_loader, criterion, optimizer,args):
    model.train()
    clip_model.train()
    epoch_time = time.time()
    loss_meter = AverageMeter()
    ...
    for step, (imgs, gt_label, imgname,bad_label) in enumerate(metric_logger.log_every(train_loader, int(batch_num/2), header)):    
        ...
        if args.use_GL :
            metric_logger.update(classifier_loss=classifier_loss.item())
            metric_logger.update(clip_loss=clip_loss.item())
        metric_logger.update(prompt_lr=lr)
