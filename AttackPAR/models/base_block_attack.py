...
class TransformerClassifier(nn.Module):
    def __init__(self, clip_model, attr_num, attributes, dim=768, pretrain_path='/media/amax/c08a625b-023d-436f-b33e-9652dc1bc7c02/DATA/kongweizhe/PARMamba/VTB-main/checkpoints/jx_vit_base_p16_224-80ecf9dd.pth'):
        super().__init__()
        super().__init__()
        ...
        # self.noisy = nn.Parameter(torch.randn(3, self.size, self.size))
        self.attack_noisy = nn.Parameter(torch.randn(3, args.height,args.width))
        

    def forward(self,imgs,clip_model):
        b_s=imgs.shape[0]

        # noisy = torch.zeros(3,args.height,args.width).cuda()
        # start_x, start_y = (args.height - self.size) // 2, (args.width - self.size) // 2
        # end_x, end_y = start_x + self.size, start_y + self.size
        # # 替换每个通道的中间区域为对应的可训练参数
        # noisy[:, start_x:end_x, start_y:end_y] = self.noisy

        imgs = imgs+self.attack_noisy
        # breakpoint()
