...
def batch_trainer(epoch, model,clip_model, train_loader, criterion, optimizer,prompt_optimizer,args):
    ...
    for step, (imgs, gt_label) in enumerate(metric_logger.log_every(train_loader, int(batch_num/2), header)):     
...
def real_trainer(model,clip_model, valid_loader,args):
    model.eval()
    loss_meter = AverageMeter()
    preds_probs = []
    gt_list = []
    with torch.no_grad():
        for step, (imgs, _) in enumerate(valid_loader):
            imgs = imgs.cuda()
            valid_logits,final_similarity = model(imgs,clip_model=clip_model)
            valid_probs = torch.sigmoid(valid_logits)
            preds_probs.append(valid_probs.cpu().numpy())
