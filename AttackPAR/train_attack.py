...
from batch_engine_attack import defense_trainer, valid_trainer, batch_trainer
from config import argument_parser
from dataset.AttrDataset import MultiModalAttrDataset, get_transform
from loss.CE_loss import *
from models.base_block_attack import *
...
def main(args):
    ...
    train_set = MultiModalAttrDataset(args=args, split=args.train_split, transform=train_tsfm,attack=True) 
    ...
    clip_test = build_model(checkpoint['clip_model']).cuda()
    ...
    for name, param in clip_test.named_parameters():
        param.requires_grad = False
...
def trainer(epoch, model, clip_model, clip_test, train_loader, valid_loader, criterion, optimizer, scheduler, prompt_optimizer, prompt_scheduler,args,path):
    min_ma,min_acc,min_f1,=1,1,1
    start=time.time()
    for i in range(1, epoch+1):
        ...
        if valid_result.ma<=min_ma :
            min_ma=valid_result.ma
            torch.save({
                        'epoch': i,
                        ...
                        'result': valid_result
                        }, os.path.join(path, f"min_ma.pth"))
        if valid_result.instance_acc<=min_acc :
            min_acc=valid_result.instance_acc
        if valid_result.instance_f1<=min_f1 :
            min_f1=valid_result.instance_f1
            torch.save({
                        'epoch': i,
                        'optimizer':optimizer.state_dict(),
                        'model_state_dict': model.state_dict(),
                        'clip_model': clip_model.state_dict(),
                        'result': valid_result
                        }, os.path.join(path, f"min_f1.pth"))
        ...
        if i%5==0:
            ...
            print(f'min_ma:{min_ma:.4f},min_acc:{min_acc:.4f},min_f1:{min_f1:.4f}') 



            # 防御
            # model, clip_model = freeze_model(model,clip_model,True)
            # train_loss, train_gt, train_probs = defense_trainer(
            #     epoch=i,
            #     model=model,
            #     clip_model=clip_model,
            #     train_loader=train_loader,
