...
from models.lora_layers import LoRALayer, LinearLoRA
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        #LoRA Define 
        # self.fc1_lora=LoRALayer(in_features, hidden_features,32,16,lora_dropout=0.05, init_lora_weights="gaussian")
        # self.fc2_lora=LoRALayer(hidden_features, out_features,32,16,lora_dropout=0.05, init_lora_weights="gaussian")
...
class Attention(nn.Module):
    def __init__(
            ...
            proj_drop=0., window_size=None, attn_head_dim=None,lora_r=32, lora_alpha=16, lora_dropout=0.05):
    ...
    def forward(self, x, enc=None, is_cross=False, rel_pos_bias=None):
        B, N, C = x.shape
        qkv_bias = None
...
class Custom_CrossAttention(nn.Module):
    def __init__(self, existing_mha):
        super(Custom_CrossAttention, self).__init__()
        self.num_heads = existing_mha.num_heads
        dim = existing_mha.dim 
        head_dim = existing_mha.dim // self.num_heads 
        if existing_mha.attn_head_dim is not None:
    ...
    def forward(self, x, enc=None, is_cross=False, rel_pos_bias=None):
        ...
        if is_cross:
            _, N2, _ = enc.shape
            k = self.k_proj(enc).reshape(B, N2, self.num_heads, -1).permute(0, 2, 1, 3)
            v = self.v_proj(enc).reshape(B, N2, self.num_heads, -1).permute(0, 2, 1, 3)     
        else:
            k = self.k_proj(x).reshape(B, N1, self.num_heads, -1).permute(0, 2, 1, 3)
            v = self.v_proj(x).reshape(B, N1, self.num_heads, -1).permute(0, 2, 1, 3)
...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None,enc=None,is_cross=False):
        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, enc=enc, is_cross=is_cross))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, enc=enc, is_cross=is_cross))
...
def apply_cross(clip_model):
    list_lora_layers = []
    for i, block in enumerate(clip_model):
        for name, submodule in block.named_children():
            if isinstance(submodule, Attention):
                new_multi_head_lora = Custom_CrossAttention(submodule)
                setattr(block, name, new_multi_head_lora)
                list_lora_layers.append(new_multi_head_lora)
    return list_lora_layers
