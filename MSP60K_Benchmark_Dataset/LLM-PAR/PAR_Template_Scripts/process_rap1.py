...
template = "The {年龄} {性别} with {身材}, {身份} {头发}, wearing {上装} on tops, and {下装} on bottoms, with {鞋子}{附件}{动作}."

attr_words = {
    "头发": ['has bald head', 'has long hair', 'has black hair', 'has hat', 'has glasses','has muffler'],
    "上装": ['shirt', 'sweater', 'vest', 'Tshirt', 'cotton top','jacket', 'suit up', 'tight top', 'short sleeve'],
    "下装": ['long trousers', 'skirt', 'short skirt', 'dress', 'jeans', 'tight trousers'],
    "鞋子": ["leather","sport shoes","boots","cloth shoes","casual shoes"],
    ...
    "身份": ['is a customer', 'is a clerk'],
    "动作": [', is calling', ', is talking',', is gathering',', is holding', ', is pushing', ', is pulling', ', is carry arm', ', is carry hand'],
}
                ...
                if attr_idx == 34:
                    ...
                    if label[attr_idx]:
                        if category in attributes.keys():
                            attributes[category] = attributes[category] + ' and ' + attr_word.replace('has ','').replace(', and with ','').replace(', and carrying ','').replace(', is ','').replace(', and ','')
                        else:
