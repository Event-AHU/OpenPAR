...
set_seed(605)
device = "cuda"
clip_model, ViT_preprocess = clip.load(
    "ViT-L/14", device=device,
    download_root='/media/amax/836e911f-c5c3-4c4b-91f2-41bb8f3f5cb6/DATA/wushujuan/PromptPAR'
)
...
def main(args):
    ...
    prompt_optimizer = torch.optim.AdamW(clip_params, lr=args.clip_lr, weight_decay=args.clip_weight_decay)
    prompt_scheduler = make_scheduler(prompt_optimizer,num_epochs=epoch_num,lr=args.clip_lr,warmup_t=10)

    optimizer = torch.optim.AdamW(mm_params, lr=args.lr, weight_decay=args.weight_decay)
