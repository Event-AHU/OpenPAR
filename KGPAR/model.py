...
assert args.dataset in ['PA100k', 'RAPV1','RAPV2','PETA','WIDER','PETAzs','RAPzs','UPAR','YCJC',"RAPV1Expand","MSP60K"], \
    f'dataset name {args.dataset} is not exist,The legal name is PA100k,RAPV1,RAPV2,PETA,WIDER,PETAzs,RAPzs,RAPV1Expand'
datasets_attrnum={'PA100k':26,'RAPV1':51,'PETA':35,'PETAzs':35,'UPAR':40,'RAPzs':53,'RAPV2':54,'WIDER':14,"RAPV1Expand":51,"MSP60K":57}
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, prompt_num:int):
        ...
        base_part_row_num = int(row_patch_num // args.div_num) + int(args.overlap_row // 2)
        ...
        self.part_end_indices = []  # 添加part_end_indices
        
        if args.use_div:
            cumulative_start = 0
            for part_idx in range(args.div_num):
                if part_idx == 0:
                    start_row = 0
                else:
                    prev_end_row = cumulative_start // row_patch_num + self.part_row_num[part_idx-1]
                    start_row = prev_end_row - int(args.overlap_row // 2)
                
                start_idx = start_row * row_patch_num
                end_idx = start_idx + self.part_num[part_idx]
                
                self.part_start_indices.append(start_idx)
                self.part_end_indices.append(end_idx)  # 初始化part_end_indices
                cumulative_start = start_row * row_patch_num
                
        if args.use_div:
            self.part_class_embedding = nn.Parameter(scale * torch.randn((args.div_num, width)))
        
        self.transformer = Transformer(width, layers, heads, VorT=True, prompt_num=prompt_num, part_num=self.part_num, row_patch_num=row_patch_num)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if args.use_div:
            x = torch.cat([x[:,:1], self.part_class_embedding.to(x.dtype) + torch.zeros(x.shape[0], args.div_num, x.shape[-1], dtype=x.dtype, device=x.device), x[:,1:]], dim=1)
        ...
        part_patch_tokens = None
        part_indices_info = None
        
        if args.use_div:
            part_patch_tokens = []
            part_indices_info = []
            
            prefix_len = self.transformer.prefix_len

            global_tokens = x[:, prefix_len:, :]
            part_patch_tokens.append(global_tokens)
            
            part_indices_info.append({
                'part_idx': -1,
                'start_idx': 0,
                'end_idx': global_tokens.shape[1],
                'shape': global_tokens.shape
            })

            for part_idx in range(args.div_num):
                original_start = self.part_start_indices[part_idx]
                original_end = min(self.part_end_indices[part_idx], global_tokens.shape[1])
                
                part_tokens = global_tokens[:, original_start:original_end, :]
                part_patch_tokens.append(part_tokens)
                
                part_indices_info.append({
                    'part_idx': part_idx,
                    'start_idx': original_start,
                    'end_idx': original_end,
                    'shape': part_tokens.shape
                })
        ...
        if self.proj is not None:
            ...
            if part_patch_tokens is not None:
                part_patch_tokens = [part_tokens @ self.proj for part_tokens in part_patch_tokens]
                    
        return x, all_class, attnmap, part_patch_tokens, part_indices_info
