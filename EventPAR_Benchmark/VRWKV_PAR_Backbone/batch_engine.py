...
def batch_trainer(epoch, model, train_loader, criterion, optimizer):
    ...
    for step, (imgs, gt_label, label_v) in enumerate(metric_logger.log_every(train_loader, int(batch_num/30), header)):  

        batch_time = time.time()
        imgs = [i.cuda() for i in imgs]
        gt_label = gt_label.cuda()
        label_v = label_v[0].cuda()
        # train_logits = model(imgs, img_temps, label_v)
        train_logits = model(imgs, label_v)
        train_loss = criterion(train_logits, gt_label)
        optimizer.zero_grad()
        train_loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 10.0) # 梯度裁剪
        optimizer.step()
...
def valid_trainer(model, valid_loader, criterion):
    ...
    gt_list = []
    with torch.no_grad():
        for step, (imgs, gt_label, label_v) in enumerate(valid_loader):
            ...
            # valid_logits = model(imgs, img_temps, label_v)
